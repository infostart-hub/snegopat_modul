...
bool loadFormForScript(SelfScript&& selfScript, string formName, IDispatch&& eventHandler, const string& eventPrefix, Value& out result) {
	...
	if (!fullPath.isEmpty()) {
		return loadScriptForm(loadFormFile(pathToForm, formName.isEmpty()?"Форма":formName), (eventHandler is null) ? cast<IDispatch>(selfScript.self) : eventHandler, eventPrefix, result, true);
	} else {
        //оставим поддержку SSF на всякий случай
		pathToForm = path;
		pathToForm.replace(RegExp(".js$"), (formName.isEmpty()?"":("." + formName))+".ssf", 1);
		return loadScriptForm(loadFormFile(pathToForm, ""), (eventHandler is null) ? cast<IDispatch>(selfScript.self) : eventHandler, eventPrefix, result, false);
	}
}
...
class ScriptRuntimeModule {
    ...
    bool call(size_t meth, Value& retVal, Vector& params, size_t
        ...
        if (pEventObject is null)
            return false;
        uint paramsCount = params.count(sizeof_ptr);
        ...
        for (uint i = 0, k = paramsCount - 1; i < paramsCount; i++) {
            args[k].setDispatch(createDispatchFromAS(&&ParamsWrapper(toValue(mem::int_ptr[params.start + i * sizeof_ptr]))));
            k--;
        }
        Variant retVar;
