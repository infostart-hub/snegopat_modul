 ...
 * Реализация работы со списком снегопата
 */
...
class IntelliSite : SmartBoxSite {
    ...
    array<SmartBoxItem&&>&& filter() {
        ...
        if (posInBuffer == 0) {
            ...
            pattern.ltrim("&\"'");
            if (pattern.find(spaceSymbol) != badStrIdx)
    ...
    void onChar(WPARAM wParam, LPARAM lParam) {
        ...
        if (' ' == symbol) {
            if (bAllowMultyFilter && ((GetKeyState(VK_SHIFT) & 0x8000) > 0 || buffer.find(spaceSymbol) != badStrIdx)) {
                symbol = spaceSymbol;
            } else {
    ...
    HWND hide(SmartBoxInsertableItem&& insert = null) {
        ...
        if (cache !is null)
            cache.clearCache();
        if (!buffer.isEmpty() && buffer.find(spaceSymbol) != badStrIdx) {
